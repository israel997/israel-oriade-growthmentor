...
export default function ConnexionPage() {
  ...
  // Si l'utilisateur est déjà connecté via Google → sync localStorage + redirect
  useEffect(() => {
    if (status === "authenticated" && session?.user) {
      const gmSession = {
        name: session.user.name ?? session.user.email ?? "Membre",
        email: session.user.email ?? "",
        photoUrl: session.user.image ?? undefined,
      };
      localStorage.setItem("gm_member_session", JSON.stringify(gmSession));
  ...
  const handleGoogleLogin = async () => {
    ...
    await signIn("google", { callbackUrl: "/connexion" });
  };
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      const s = { name: loginEmail.split("@")[0], email: loginEmail.trim() };
      localStorage.setItem("gm_member_session", JSON.stringify(s));
      router.push("/espace-membre");
    }, 600);
  };
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      const s = { name: regName.trim(), email: regEmail.trim() };
      localStorage.setItem("gm_member_session", JSON.stringify(s));
      router.push("/espace-membre");
    }, 600);
  };
  ...
  if (status === "loading") return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "#03071A" }}>
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#03071A" }}>
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 overflow-hidden">
        <div style={{
          ...
          left: "-10%", top: "-10%", borderRadius: "50%",
          background: "radial-gradient(circle at 40% 40%, rgba(26,63,216,0.4) 0%, transparent 70%)",
          filter: "blur(60px)",
        }} />
        <div style={{
          position: "absolute", width: "50vw", height: "50vw", maxWidth: 500, maxHeight: 500,
          right: "-5%", bottom: "0%", borderRadius: "50%",
          background: "radial-gradient(circle at 60% 60%, rgba(139,92,246,0.3) 0%, transparent 70%)",
          filter: "blur(55px)",
        }} />
      </div>
        ...
        <div className="rounded-3xl p-8"
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest mb-2"
                  style={{ color: "rgba(255,255,255,0.35)" }}>Adresse email</label>
          ...
          {tab === "register" && (
            <form onSubmit={handleRegister} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest mb-2"
                  style={{ color: "rgba(255,255,255,0.35)" }}>Nom complet</label>
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest mb-2"
                  style={{ color: "rgba(255,255,255,0.35)" }}>Confirmer le mot de passe</label>
